...
final class SemaphoreImpl implements Semaphore {
    ...
    public final boolean addAcquireToQueue(CancellableContinuation<? super Unit> cont) {
        ...
        Object objM1507constructorimpl;
        Object s$iv;
        boolean z;
            ...
            while (true) {
                if (cur$iv$iv.getId() < id$iv || cur$iv$iv.getRemoved()) {
                    ...
                    if (it$iv$iv$iv == ConcurrentLinkedListKt.CLOSED) {
                        objM1507constructorimpl = SegmentOrClosed.m1507constructorimpl(ConcurrentLinkedListKt.CLOSED);
                        break;
                    }
                    ...
                    objM1507constructorimpl = SegmentOrClosed.m1507constructorimpl(cur$iv$iv);
                    curTail = s$iv2;
                    break;
                }
            }
            s$iv = objM1507constructorimpl;
            if (SegmentOrClosed.m1512isClosedimpl(s$iv)) {
                break;
            }
            Segment to$iv$iv = SegmentOrClosed.m1510getSegmentimpl(s$iv);
            while (true) {
                ...
                if (tail$FU.compareAndSet(this, cur$iv$iv2, to$iv$iv)) {
        ...
        SemaphoreSegment segment = (SemaphoreSegment) SegmentOrClosed.m1510getSegmentimpl(s$iv);
        int i = (int) (enqIdx % ((long) SemaphoreKt.SEGMENT_SIZE));
        if (segment.acquirers.compareAndSet(i, null, cont)) {
    ...
    private final boolean tryResumeNextFromQueue() {
        ...
        do {
            ...
            while (true) {
                ...
                if (head$FU.compareAndSet(this, cur$iv$iv2, to$iv$iv)) {
                    if (cur$iv$iv2.decPointers$kotlinx_coroutines_core()) {
                        cur$iv$iv2.remove();
                    }
                    z = true;
        ...
        if (value$iv == null) {
            ...
            return !segment.acquirers.compareAndSet(i, SemaphoreKt.PERMIT, SemaphoreKt.BROKEN);
        }
