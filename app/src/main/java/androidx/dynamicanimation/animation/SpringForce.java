...
public final class SpringForce implements Force {
    ...
    DynamicAnimation.MassState updateValues(double lastDisplacement, double lastVelocity, long timeElapsed) {
        ...
        if (displacement2 > 1.0d) {
            ...
            double dPow = coeffA * d4 * Math.pow(2.718281828459045d, d4 * deltaT);
            double d5 = this.mGammaPlus;
            double currentVelocity = dPow + (coeffB * d5 * Math.pow(2.718281828459045d, d5 * deltaT));
            ...
            double currentVelocity2 = (dPow2 * (-d7)) + (Math.pow(2.718281828459045d, (-d7) * deltaT) * coeffB2);
            ...
            double dPow3 = Math.pow(2.718281828459045d, (-lastDisplacement3) * d10 * deltaT);
            double d12 = this.mDampedFreq;
            double dSin = (-d12) * lastDisplacement2 * Math.sin(d12 * deltaT);
            double d13 = this.mDampedFreq;
            double dCos = d11 + (dPow3 * (dSin + (d13 * sinCoeff * Math.cos(d13 * deltaT))));
